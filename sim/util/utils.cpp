...
bool write_and_compare(const CaseConfig &cfg, const std::vector<AccType> &C,
    ...
    if (!diffs_idx.empty()) {
        emit_report(cfg, diffs_idx, std::vector<int32_t>(C.begin(), C.end()), Cgold, A, B);
        return false;
    }
    return true;
}
...
std::vector<size_t> compute_diffs(const std::vector<int32_t>& got, const std::vector<int32_t>& expected) {
    ...
    if (got.size() != expected.size()) {
        // mark trailing indices as diffs
...
void emit_report(const CaseConfig &cfg, const std::vector<size_t> &diffs_idx,
    ...
    for (size_t d = 0; d < show; ++d) {
        ...
        std::cerr << "diff[" << d << "] idx=" << idx << " (r=" << row << ",c=" << col << ") expected=" << expected[idx] << " got=" << got[idx] << "\n";
        std::cerr << "  A[row] = [";
    ...
    if (df) {
        ...
        for (size_t idx : diffs_idx) {
            size_t row = cfg.M > 0 ? idx / cfg.N : 0;
            size_t col = cfg.N > 0 ? idx % cfg.N : 0;
            df << idx << "," << row << "," << col << ",got=" << got[idx] << ",exp=" << expected[idx] << "\n";
            df << "A_row:";
